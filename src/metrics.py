...
def mia(
    ...
    # Try to construct LogisticRegression with multi_class; fall back if unsupported
    try:
        clf = LogisticRegression(class_weight="balanced", solver="lbfgs", multi_class="multinomial", max_iter=1000)
    except TypeError:
        print("LogisticRegression does not support 'multi_class' parameter in this sklearn; falling back to default constructor")
        clf = LogisticRegression(class_weight="balanced", solver="lbfgs", max_iter=1000)

    # Debug info
    print(f"MIA: X_r.shape={X_r.shape}, Y_r.shape={Y_r.shape}, X_f.shape={X_f.shape}")
